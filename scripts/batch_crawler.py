...
def is_cloudflare_page(result) -> bool:
    title = ""
    try:
    ...
    hay = (title + "\n" + content).lower()
    return any(m.lower() in hay for m in CLOUDFLARE_MARKERS)
...
async def crawl_batch(urls: List[str], max_concurrent: int = 5):
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        for idx, url in enumerate(urls, start=1):
            # sleep BEFORE each request (best for Cloudflare)
            delay_s = 10
            print(f"⏳ [{idx}/{len(urls)}] Sleeping {delay_s}s before crawling...")
